import pytest
from crud.player import get_player, get_players, create_player, delete_player
from schemas.schemas import PlayerCreate

def test_create_basic_player(db_session):
    player = create_player(db_session, PlayerCreate(
        nick="bob",
        password="test123"
    ))
...
def test_get_existing_player(db_session):
    created = create_player(db_session, PlayerCreate(
        nick="alice",
        password="test123"
    ))
